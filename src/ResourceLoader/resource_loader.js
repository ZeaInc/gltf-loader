...
class ResourceLoader {
  ...
  async loadGltf(gltfFile, externalFiles) {
    ...
    if (typeof gltfFile === 'string') {
      ...
      let data = await resourceLoader.loadFile(isGlb ? 'binary' : 'json', gltfFile)
      json = data
      data = data
      filename = gltfFile
...
async function _loadEnvironmentFromPanorama(imageHDR, view, luts) {
  ...
  environment.samplers.push(
    new gltfSampler(GL.LINEAR, GL.LINEAR, GL.CLAMP_TO_EDGE, GL.CLAMP_TO_EDGE, 'DiffuseCubeMapSampler')
  )
  const diffuseCubeSamplerIdx = samplerIdx++

  environment.samplers.push(
    new gltfSampler(GL.LINEAR, GL.LINEAR_MIPMAP_LINEAR, GL.CLAMP_TO_EDGE, GL.CLAMP_TO_EDGE, 'SpecularCubeMapSampler')
  )
  const specularCubeSamplerIdx = samplerIdx++

  environment.samplers.push(
    new gltfSampler(GL.LINEAR, GL.LINEAR_MIPMAP_LINEAR, GL.CLAMP_TO_EDGE, GL.CLAMP_TO_EDGE, 'SheenCubeMapSampler')
  )
  ...
  const diffuseTexture = new gltfTexture(diffuseCubeSamplerIdx, [imageIdx++], GL.TEXTURE_CUBE_MAP)
  ...
  const specularTexture = new gltfTexture(specularCubeSamplerIdx, [imageIdx++], GL.TEXTURE_CUBE_MAP)
  ...
  environment.images.push(
    new gltfImage(luts.lut_sheen_E_file, GL.TEXTURE_2D, 0, undefined, undefined, ImageMimeType.PNG)
  )
