...
class gltfRenderer {
  constructor(context) {
    ...
    let requiredWebglExtensions = ['EXT_texture_filter_anisotropic', 'OES_texture_float_linear']
  ...
  drawScene(state, scene) {
    ...
    const drawables = nodes
      ...
      .reduce(
        (acc, node) =>
          acc.concat(
            state.gltf.meshes[node.mesh].primitives.map((primitive) => {
    ...
    const opaqueDrawables = drawables.filter(
      ({ node, primitive }) =>
    ...
    let transparentDrawables = drawables.filter(
      ({ node, primitive }) =>
        state.gltf.materials[primitive.material].alphaMode === 'BLEND' &&
        (state.gltf.materials[primitive.material].extensions === undefined ||
          state.gltf.materials[primitive.material].extensions.KHR_materials_transmission === undefined)
    ...
    let transmissionDrawables = drawables.filter(
      ({ node, primitive }) =>
        state.gltf.materials[primitive.material].extensions !== undefined &&
        state.gltf.materials[primitive.material].extensions.KHR_materials_transmission !== undefined
    )
  ...
  drawPrimitive(state, primitive, node, viewProjectionMatrix, transmissionSampleTexture) {
    ...
    for (let i = 0; i < material.textures.length; ++i) {
      ...
      if (!this.webGl.setTexture(location, state.gltf, info, i)) {
    ...
    if (
      ...
      (state.renderingParameters.useIBL || state.renderingParameters.usePunctual) &&
      state.environment &&
