...
class gltfMaterial extends GltfObject {
  ...
  static createDefault() {
    ...
    const baseColorFactor = new Vec4(1, 1, 1, 1)
  ...
  parseTextureInfoExtensions(textureInfo, textureKey) {
    ...
    if (textureInfo.extensions.KHR_texture_transform !== undefined) {
      ...
      let scale = new Mat3()
      let translation = new Mat3()

      if (uvTransform.rotation !== undefined) {
        const s = Math.sin(uvTransform.rotation)
        const c = Math.cos(uvTransform.rotation)

        rotation = new Mat3([c, s, 0.0, -s, c, 0.0, 0.0, 0.0, 1.0])
      }

      if (uvTransform.scale !== undefined) {
        scale = new Mat3([uvTransform.scale[0], 0, 0, 0, uvTransform.scale[1], 0, 0, 0, 1])
      }

      if (uvTransform.offset !== undefined) {
        translation = new Mat3([1, 0, uvTransform.offset[0], 0, 1, uvTransform.offset[1], 0, 0, 1])
      }

      //   let uvMatrix = new Mat3()
      const uvMatrix = rotation.multiply(scale)
      uvMatrix.multiplyInPlace(translation)
  ...
  initGl(gltf, webGlContext) {
    ...
    if (this.pbrMetallicRoughness !== undefined && this.type !== 'SG') {
      ...
      if (this.pbrMetallicRoughness.baseColorFactor !== undefined) {
        baseColorFactor.set(...this.pbrMetallicRoughness.baseColorFactor)
      }
      ...
      this.zeaMaterial.getParameter('BaseColor').setValue(baseColorFactor)
      this.zeaMaterial.getParameter('Metallic').setValue(metallicFactor)
      this.zeaMaterial.getParameter('Roughness').setValue(roughnessFactor)
    }
